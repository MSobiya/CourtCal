...
//-----------We check for 5 because we consider in a court there are 5 rooms so that at a single time 5 cases can be schedule and maximum scheduling time is greater than current time

if($rows < 5 and $rows > 0 and $r[0]['scheduling_time'] > date('Y-m-d H:i:s')){
...
//--Delete Previous Schedule if any for particular case it is helpful only when Judge Reschedule the case---

$sql = "DELETE FROM Case_Scheduling where case_id='$case_id'";
$result = mysqli_query($con,$sql); 

//---insert into Case Scheduling table
